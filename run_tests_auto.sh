...
if [ -n "$CURRENT_LEADER" ]; then
    ...
    INITIAL_COUNT=$(grep -c "Heartbeat from worker" "$LEADER_LOG" 2>/dev/null || echo 0)
    echo "  Measuring throughput over 10 seconds..."
    sleep 10
    FINAL_COUNT=$(grep -c "Heartbeat from worker" "$LEADER_LOG" 2>/dev/null || echo 0)
...
echo -e "\n${CYAN}Test: Leader Election Latency${NC}"
if [ -n "$CURRENT_LEADER_INDEX" ]; then
    echo "  Triggering leader failure to measure election time..."
    
    OLD_LEADER_PID=${MANAGER_PIDS[$CURRENT_LEADER_INDEX]}
    START_TIME=$(date +%s)
    
    kill -9 "$OLD_LEADER_PID" 2>/dev/null || true
    echo "  Killed current leader at $(date +%H:%M:%S)"
    ...
    for attempt in {1..15}; do
        sleep 1
        for i in ${!NODES[@]}; do
            if [ "$i" != "$CURRENT_LEADER_INDEX" ] && ps -p ${MANAGER_PIDS[$i]} > /dev/null 2>&1; then
                RAFT_LOG="node_${NODES[$i]}.log"
                if tail -20 "$RAFT_LOG" | grep -q "Transition -> LEADER"; then
                    END_TIME=$(date +%s)
                    ELECTION_TIME=$((END_TIME - START_TIME))
                    echo -e "  New leader elected at $(date +%H:%M:%S)"
                    echo -e "  Election latency: ${ELECTION_TIME} seconds"
                    
                    if [ "$ELECTION_TIME" -le 5 ]; then
                        echo -e "${GREEN}✓ PASS: Fast leader election (${ELECTION_TIME}s)${NC}"
                    else
                        echo -e "${GREEN}✓ PASS: Leader election completed (${ELECTION_TIME}s)${NC}"
                    fi
                    ELECTED=true
                    break 2
                fi
    ...
    if [ "$ELECTED" = false ]; then
        echo -e "${YELLOW}⚠ Election took longer than expected${NC}"
    fi
fi
...
echo -e "\n${CYAN}Test: Log Replication Latency${NC}"
echo -e "${YELLOW}Measuring replication delay from leader to followers...${NC}"

# Get timestamp of latest leader commit
for i in ${!NODES[@]}; do
    if ps -p ${MANAGER_PIDS[$i]} > /dev/null 2>&1; then
        RAFT_LOG="node_${NODES[$i]}.log"
...
echo -e "${GREEN}✓ PASS: Replication latency measurement complete${NC}"
